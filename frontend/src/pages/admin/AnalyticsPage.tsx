  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Platform metrics and trends</p>
          </div>
        </div>
        <button
          onClick={fetchAnalytics}
          className="flex items-center gap-1 text-sm text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 transition"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <SummaryCard icon={<TrendingUp size={18} />} label="Completed" value={summary.completedJobs} color="text-green-600 dark:text-green-400" />
        <SummaryCard icon={<AlertTriangle size={18} />} label="Failed" value={summary.failedJobs} color="text-red-600 dark:text-red-400" />
        <SummaryCard icon={<IndianRupee size={18} />} label="Revenue" value={`₹${(summary.totalRevenue / 100).toFixed(2)}`} color="text-emerald-600 dark:text-emerald-400" />
      ...
      {daily.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-5">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Daily Jobs (Last 30 Days)</h3>
          <div className="flex items-end gap-1 h-40">
            {(() => {
              const maxJobs = Math.max(...daily.map(d => d.jobs), 1);
              return daily.map((d) => (
                <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                  <div className="absolute bottom-full mb-1 hidden group-hover:block bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-800 text-xs px-2 py-1 rounded whitespace-nowrap z-10 shadow-lg">
                    {d.date}: {d.jobs} jobs, ₹{(d.revenue / 100).toFixed(0)}, {d.pages} pages
                  </div>
                  <div
                    className="w-full bg-primary-500 dark:bg-primary-400 rounded-t hover:bg-primary-600 dark:hover:bg-primary-300 transition-colors min-h-[2px]"
          ...
          {(() => {
            ...
            return hourly.map((h) => (
              <div key={h.hour} className="flex items-center gap-2 text-xs">
                ...
                <span className="w-8 text-gray-600 dark:text-gray-300 font-medium">{h.jobs}</span>
              </div>
            ));
          })()}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                return statusBreakdown.map((s) => (
                  <div key={s.status}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="text-gray-500 dark:text-gray-400 font-medium">{s.count} ({Math.round((s.count / total) * 100)}%)</span>
                    </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-5">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Payment Methods</h3>
          {paymentTypeBreakdown.length === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-500">No payment data</p>
          ) : (
            <div className="space-y-3">
              {paymentTypeBreakdown.map((p) => (
                <div key={p.type} className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-sm text-gray-700 dark:text-gray-200 capitalize">{p.type || 'Unknown'}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500">{p.count} payments</span>
                  </div>
                  <p className="text-lg font-semibold text-gray-900 dark:text-white">₹{(p.amount / 100).toFixed(2)}</p>
                </div>
              ))}
            </div>
          )}
...
function SummaryCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string | number; color?: string }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-4">
      <div className={`mb-1 ${color || 'text-gray-400 dark:text-gray-500'}`}>{icon}</div>
      <p className="text-xs text-gray-500 dark:text-gray-400">{label}</p>
      <p className={`text-lg font-bold ${color || 'text-gray-800 dark:text-gray-100'}`}>{value}</p>
    </div>
  );
}
